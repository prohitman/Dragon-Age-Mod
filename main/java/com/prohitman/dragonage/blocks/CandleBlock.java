...
public class CandleBlock extends Block
{
	...
	protected static final VoxelShape ONE_CANDLE = Stream.of(
	...
	protected static final VoxelShape TWO_CANDLES =
			Stream.of(
					Block.makeCuboidShape(3.5, 0, 4.5, 6.5, 5, 7.5),
					...
					Block.makeCuboidShape(9.83397, 7, 9.17772, 10.83397, 9, 9.17772),
					Block.makeCuboidShape(6, 0, 9, 9, 7, 12)
					).reduce((v1, v2) -> {return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);}).get();
	
	protected static final VoxelShape THREE_CANDLES = Stream.of(
			Block.makeCuboidShape(3.5, 0, 4.5, 6.5, 5, 7.5),
			Block.makeCuboidShape(6.8795732188134515, 5.00314, 4.368216094067261, 7.8795732188134515, 7.00314, 4.368216094067261),
			Block.makeCuboidShape(5.687733230091756, 7, 9.96, 6.687733230091756, 9, 9.96),
			Block.makeCuboidShape(4.061006094067261, 5.00314, 8.610396781186548, 5.06100609406726, 7.00314, 8.610396781186548),
			Block.makeCuboidShape(9.83396895112973, 7, 9.1777200066867, 10.833968951129728, 9, 9.1777200066867),
			Block.makeCuboidShape(6, 0, 9, 9, 7, 12),
			Block.makeCuboidShape(9.5, 0, 5.5, 12.5, 6, 8.5),
	...
	public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
	      return useContext.getItem().getItem() == this.asItem() && state.get(CANDLES) < 3 ? true : super.isReplaceable(state, useContext);
	}
